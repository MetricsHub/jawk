 ...
 * AWK: conversions must match the value's Java type. AWK integral numbers
 * reach the sink as {@link Long}, other numbers as {@link Double}, and text
 * as {@link String}, so {@code %d} requires an integral value and {@code %f}
 * a floating-point one; a mismatch raises Java's
 * {@link java.util.IllegalFormatException}. Input-derived values and
 * uninitialized variables are passed as plain strings.
 * </p>
 */
public class JavaStringFormatAwkSink extends OutputStreamAwkSink {
	...
	public String sprintf(String convfmt, String format, Object... values) {
		...
		for (int i = 0; i < safeValues.length; i++) {
			...
			// Jawk-internal scalar types are exposed as plain strings.
			if (value instanceof StrNum || value instanceof UninitializedObject) {
